...
bool check(int u, lli k) {
	...
	while(!q.empty()) {
		int v = q.front();
		q.pop();
		if(level[v] == 0 && adj[v].size() < 3) return false;
		if(level[v] <= k-1 && level[v] != 0 && adj[v].size() < 4) return false;
		if(level[v] == k && adj[v].size() > 1) return false;
		fore(i, 0, adj[v].size()) {
			if(adj[v][i] != parent[v]) {
				q.push(adj[v][i]);
				parent[adj[v][i]] = v;
